...
export const signUp = async (req, res, next) => {
    try {
        ...
        client.set(`user:${data._id}`, JSON.stringify(data), (err, reply) => {
            if (err) {
                console.log(err);
                return next(new errorClass("failed to store user data in Redis", 500));
            }
            res.json({
                sucess: true,
    ...
    export const getMyProfile = async (req, res, next) => {
        try {
            ...
            const user = await User.findById(id, "userName name email avatar.url");
            if (!user)
