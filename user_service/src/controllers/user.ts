...
export const signUp=async(req:Request<{},{},{name:string,username:string,email:string,password:string}>,res:Response,next:NextFunction)=>{
   try{
    ...
    console.log(error)
        if(error.code===11000 && "email" in error.keyValue) return next(new errorClass("email already eists",400));
        if(error.code===11000 && "username" in error.keyValue) return next(new errorClass("username already eists",400))
...
export const login=async(req:Request<{},{},{email:string,password:string}>,res:Response,next:NextFunction)=>{
    ...
    export const getMyProfile=async(req:Request,res:Response,next:NextFunction)=>{
       try {const {id}=(req as any).user;
        if(await client.exists(`user:${id}`)){
            res.send(await client.get(`user:${id}`));
        }
        else{
        if(!id) return next(new errorClass("please login first",401));
...
export const updateUser=async(req:Request<{},{},{name:string,email:string,username:string}>,res:Response,next:NextFunction)=>{
    try{
        const {name,username,email}=req.body;
        const {id}=(req as any).user;
        if(!id) return next(new errorClass("please login first",401));
        const user=await User.findById(id);
        if(!user) return next(new errorClass("please login first",401));
        const file=req.file;
        if(!name && !username && email && file) return next(new errorClass("please enter any field to change",400));
        const  query:{
            name?:string,
            email?:string,
            file?:{url:string}
...
export const deleteUser=async(req:Request,res:Response,next:NextFunction)=>{
    try{
        ...
        if(!channel)  return next(new errorClass("channel not found",400));
...
export const addAddress=async(req:Request<{},{},{ street:string,pinCode:number,city:string,state:string,nearByPlace:string}>
