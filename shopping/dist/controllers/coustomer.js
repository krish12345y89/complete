import { getChannel, rpcRequest, subscribeMessage } from "../features/micro_utils.js";
...
export const getMyList = async (req, res, next) => {
    try {
        const { id } = req.user;
        if (!id)
            return next(new errorClass("You must be logged in to add items to your list", 401));
        const cashedData = await client.get(`list:${id}`);
        res.json(JSON.parse(cashedData));
        if (!cashedData) {
            const list = await wishList.findOne({ user: id });
            if (!list)
                return next(new errorClass("Please create wishlist first", 400));
...
export const handleSubscribeEvent = async (req, res, next, payload) => {
    try {
        const { data, event, msg } = payload;
        switch (event) {
            case "delete_profile":
...
export const deleteProfile = async (next, data, res) => {
    try {
        const { id } = data;
        ...
        if (!cart)
            return next(new errorClass("cart not found", 400));
        if (await client.exists(`cart:id`))
            await client.delete(`cart:${id}`);
        const list = await wishList.findOneAndDelete({ user: id });
        if (!list)
            return next(new errorClass("list not found", 400));
        if (await client.exists(`list:id`))
