...
// Constants
const msgQueueUrl = 'amqp://localhost';
...
export const subscribeMessage = async (channel: amqplib.Channel, service: { SubscribeEvents: (message: string) => void }): Promise<void> => {
  await channel.assertExchange(exchangeName, "direct", { durable: true });
  const q = await channel.assertQueue("", { exclusive: true });
  console.log(`Waiting for messages in queue: ${q.queue}`);

  channel.bindQueue(q.queue, exchangeName, productService);
